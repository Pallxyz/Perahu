<x-app-layout>
    ...
    x-init="applyTheme(currentTheme); connectWS(); setTimeout(() => initChart(), 500);"
        ...
        <div class="flex flex-col md:flex-row min-h-screen">
                ...
                <div>
                    ...
                    <nav class="space-y-2">
                        <button @click="activeTab = 'control'" 
                                :class="activeTab === 'control' ? 'bg-indigo-600 text-white font-semibold shadow-md shadow-indigo-500/20' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800/60 hover:text-indigo-600 dark:hover:text-indigo-400'" 
                        ...
                        <button @click="activeTab = 'telemetry'" 
                                :class="activeTab === 'telemetry' ? 'bg-indigo-600 text-white font-semibold shadow-md shadow-indigo-500/20' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800/60 hover:text-indigo-600 dark:hover:text-indigo-400'" 
                                class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition duration-200">
                        ...
                        <button @click="activeTab = 'map'" 
                                :class="activeTab === 'map' ? 'bg-indigo-600 text-white font-semibold shadow-md shadow-indigo-500/20' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800/60 hover:text-indigo-600 dark:hover:text-indigo-400'" 
                                class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition duration-200">
                        ...
                        <button @click="activeTab = 'system'" 
                                :class="activeTab === 'system' ? 'bg-indigo-600 text-white font-semibold shadow-md shadow-indigo-500/20' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800/60 hover:text-indigo-600 dark:hover:text-indigo-400'" 
                                class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition duration-200">
            ...
            <main class="flex-1 p-6 overflow-y-auto">
                <div class="max-w-6xl mx-auto">
                    ...
                    <!-- Connection Bar -->
                    <div class="bg-white dark:bg-[#1a1a2e] border border-gray-200 dark:border-gray-800 rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 transition-colors duration-300">
                        <div class="flex items-center gap-2">
                            <span class="w-2.5 h-2.5 rounded-full" :class="wsConnected ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'"></span>
                            <span class="text-sm font-medium text-gray-600 dark:text-gray-300" x-text="wsConnected ? 'Terhubung ke ESP32' : 'Terputus dari ESP32'"></span>
                        </div>
                        <div class="flex items-center gap-2 w-full sm:w-auto">
                            <input x-model="esp32Ip" type="text" placeholder="IP ESP32" class="text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#2d2d44] text-gray-800 dark:text-gray-100 px-3 py-1.5 w-full sm:w-56 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
                            <button type="button" @click="connectWS()" class="text-sm px-4 py-1.5 rounded-xl text-white font-semibold bg-indigo-600 hover:bg-indigo-700 transition whitespace-nowrap">Connect & Simpan</button>
                        </div>
                    </div>
